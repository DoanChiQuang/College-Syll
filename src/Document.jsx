...
const MyDocument = () => {
    ...
    const value = location.state.syll;    
                            ...
                            <View style={{flexDirection: 'row'}}>
                                ...
                                <View style={{flex: 1}}>
                                    <View style={styles.sentence}>
                                        ...
                                        <Text style={styles.answer}>{value.realname}</Text>
                                    </View>
                                    <View style={styles.sentence}>
                                        <Text style={styles.question}>
                                            2. Tên gọi khác:
                                        </Text>
                                        <Text style={styles.answer}>{value.nickname}</Text>
                                    </View>
                                    <View style={styles.sentence}>
                                        <Text style={styles.question}>3. Sinh ngày:</Text>
                                        <Text style={styles.answer}>{dayjs(value.birthday).format("DD/MM/YYYY")}</Text>
                                    </View>
                                    <View style={styles.sentence}>
                                        <Text style={styles.question}>
                                            4. Nơi sinh: Phường/Xã:
                                        </Text>
                                        <Text style={styles.answer}>{value.placeOfBirth.address.ward.find(item => item.id == value.placeOfBirth.wardId).label}</Text>
                                        <Text style={styles.question}>Quận/Huyện:</Text>
                                        <Text style={styles.answer}>{value.placeOfBirth.address.district.find(item => item.id == value.placeOfBirth.districtId).label}</Text>
                                        <Text style={styles.question}>TP/Tỉnh:</Text>
                                        <Text style={styles.answer}>{value.placeOfBirth.address.province.find(item => item.id == value.placeOfBirth.provinceId).label}</Text>
                                    </View>
                                    <View style={styles.sentence}>
                                        <Text style={styles.question}>
                                            5. Quê quán: Phường/Xã:
                                        </Text>
                                        <Text style={styles.answer}>{value.homeTown.address.ward.find(item => item.id == value.homeTown.wardId).label}</Text>
                                        <Text style={styles.question}>Quận/Huyện:</Text>
                                        <Text style={styles.answer}>{value.homeTown.address.district.find(item => item.id == value.homeTown.districtId).label}</Text>
                                        <Text style={styles.question}>TP/Tỉnh:</Text>
                                        <Text style={styles.answer}>{value.homeTown.address.province.find(item => item.id == value.homeTown.provinceId).label}</Text>
                                    </View>
                                </View>
                            </View>
                            <View style={styles.sentence}>
                                <Text style={styles.question}>6. Dân tộc:</Text>
                                <Text style={styles.answer}>{value.nation.data.find(item => item.id == value.nation.nationId).label}</Text>
                                <Text style={styles.question}>7. Tôn giáo:</Text>
                                <Text style={styles.answer}>{value.religion.data.find(item => item.id == value.religion.religionId).label}</Text>
                            </View>
                            <View style={styles.sentence}>
                                <Text style={styles.question}>
